...
const traverseHtmlElements = (rootNode, callback) => {
  traverseHtmlNode(rootNode, node => {
    if (!plateCore.isHtmlElement(node)) {
      return true;
    }
...
const traverseHtmlComments = (rootNode, callback) => {
  traverseHtmlNode(rootNode, node => {
    if (!plateCore.isHtmlComment(node)) {
      return true;
    }
...
const removeHtmlNodesBetweenComments = (rootNode, start, end) => {
  const isClosingComment = node => plateCore.isHtmlComment(node) && node.data === end;
...
const getListNode = type => element => {
  ...
  if (element.style.textAlign) {
    node[plateAlignment.KEY_ALIGN] = element.style.textAlign;
  }
...
const createDeserializeDocxPlugin = plateCore.createPluginFactory({
  ...
  overrideByKey: {
    ...
    [plateCodeBlock.ELEMENT_CODE_BLOCK]: {
      deserializeHtml: [{
        validNodeName: 'PRE',
        ...plateCodeBlock.deserializeHtmlCodeBlock
      }, {
