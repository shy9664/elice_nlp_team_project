...
const autoformatBlock = (editor, {
  ...
  for (const match of matches) {
    ...
    if (triggerAtBlockStart) {
      ...
      const textFromBlockStart = plateCore.getText(editor, matchRange);
...
const getMatchPoints = (editor, {
  ...
  if (end) {
    beforeEndMatchPoint = plateCore.getPointBefore(editor, selection, {
      matchString: end
    });
  ...
  if (start) {
    afterStartMatchPoint = plateCore.getPointBefore(editor, beforeEndMatchPoint, {
      matchString: start,
      skipInvalid: true,
      afterMatch: true
    });
    if (!afterStartMatchPoint) return;
    beforeStartMatchPoint = plateCore.getPointBefore(editor, beforeEndMatchPoint, {
      matchString: start,
      skipInvalid: true
    });
...
const autoformatMark = (editor, {
  ...
  for (const match of matches) {
    ...
    if (!ignoreTrim) {
      const matchText = plateCore.getText(editor, matchRange);
